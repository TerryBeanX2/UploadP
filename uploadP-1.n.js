!function () {
    ...
    //有关联的参数：useCurrentSize 设置为true时，getCurrentSize 函数必须被重写,暂停按钮pauseBtn生效
    ...
    function UploadP(option) {
        this.o = {
            ...
            oneComplete: function (upLoadPFileId) {
    ...
    UploadP.prototype = {
        ...
        init: function () {
            ...
            this.Event.on('startUpload', function () {
                // if(!window.startedTime) window.startedTime = 0;
                // window.startedTime++;
            ...
            this.Event.on('oneComplete', function (upLoadPFIleId) {
                            ...
                            if (item.xhr.status == 200) {
                                ...
                                if (self.dataFilter && (data[self.dataFilter[0]] == self.dataFilter[1]) || !self.dataFilter) {
                                    ...
                                    if (item.currentChunk == item.chunks - 1) {
                                        item.complete = true;
                                        item.uploading = false;
                                        self.Event.emit('oneComplete', item.upLoadPFileId);
                                        self.Event.emit('startUpload');
                                        return
                                    }
